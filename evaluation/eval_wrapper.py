...
def call_culane_eval(data_dir, exp_name,output_path):
    ...
    im_h=590
    frame=1
    test_list = os.path.join(data_dir,'list/test.txt')
    ...
    test_out = os.path.join(output_path,'txt','test_out.txt')
    ...
    os.system('%s -a %s -d %s -i %s -l %s -w %s -t %s -c %s -r %s -f %s -o %s'%(eval_cmd,data_dir,detect_dir,data_dir,test_list,w_lane,iou,im_w,im_h,frame,test_out))
    ...
    res_all = {}
    res_all['Result:'] = read_helper(test_out)
